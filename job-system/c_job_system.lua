...
function showEmploymentWindow()
	...
	-- Wood
	local row = guiGridListAddRow(jobList)
	guiGridListSetItemText(jobList, row, column, "Wood Cutter", false, false)
	...
	if (button=="left") then
		...
		elseif (job>0) then
			...
			elseif (jobtext=="Locksmith") then
				displayLocksmithJob()
				job = 6
			elseif (jobtext == "Wood Cutter") then 
				outputChatBox("You can take wood cutter job at the Yellow icon on your GPS", 255, 0, 0)
			end
