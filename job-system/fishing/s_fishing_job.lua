	...
	if (isElement(thePlayer)) then
		...
		if (logged==1) then
			...
			if not(exports.global:hasItem(thePlayer, 49)) then -- does the player have the fishing rod item?
				exports.hud:sendBottomNotification(thePlayer, "Fishing", "You need a fishing rod to fish.")
			else
...
function theyHaveABite()
	exports.global:sendLocalMeAction(source,"'s line starts to jerk around.")
end
...
function lineSnap() ----- Snapped line.
	...
	triggerEvent("artifacts:remove", source, source, "rod")
...
function catchFish(fishSize, totalCatch)
	exports.global:sendLocalMeAction(client,"reels in their line, catching a fish at ".. fishSize .."lbs.")
...
------ /sellfish
function unloadCatch( totalCatch )
	...
	exports.global:giveMoney(source, math.floor( totalCatch * 3 ))
...
addEventHandler("onCharacterLogin", getRootElement(), fishingNotice)